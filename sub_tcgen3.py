...
// Tile storage (1024-byte aligned for descriptor base-offset)
...
// ============================================================================
// TMA LOAD - Issue multiple cp.async.bulk to gather strided rows into contiguous temp
// ============================================================================
#define BYTES_PER_ROW (MMA_K / 2)  // 32 bytes per row

__device__ __forceinline__ void tma_load_a_strided(
    const Gemm_params& params, int m_block, int k_offset,
    uint32_t temp_smem_addr, uint32_t mbar_smem)
{
    const uint8_t* a_base = reinterpret_cast<const uint8_t*>(params.a_ptr)
                            + m_block * params.a_row_stride + k_offset / 2;

    if (threadIdx.x == 0) {
        // Arrive + set expected bytes for all rows
        asm volatile(
            "mbarrier.arrive.expect_tx.shared::cta.b64 _, [%0], %1;"
            :: "r"(mbar_smem), "r"(SMEM_A_TILE) : "memory"
        );

        // Issue cp.async.bulk for each row (32 bytes each, strided in global, contiguous in smem)
        #pragma unroll
        for (int m = 0; m < MMA_M; m++) {
            const uint8_t* src = a_base + m * params.a_row_stride;
            uint32_t dst = temp_smem_addr + m * BYTES_PER_ROW;
            asm volatile(
                "cp.async.bulk.shared::cta.global.mbarrier::complete_tx::bytes [%0], [%1], %2, [%3];"
                :: "r"(dst), "l"(src), "r"(BYTES_PER_ROW), "r"(mbar_smem) : "memory"
            );
        }
    }
}

__device__ __forceinline__ void tma_load_b_strided(
    const Gemm_params& params, int n_block, int k_offset,
    uint32_t temp_smem_addr, uint32_t mbar_smem)
{
    const uint8_t* b_base = reinterpret_cast<const uint8_t*>(params.b_ptr)
    ...
    if (threadIdx.x == 0) {
        ...
        // Issue cp.async.bulk for each row
        #pragma unroll
        for (int n = 0; n < MMA_N; n++) {
            const uint8_t* src = b_base + n * params.b_row_stride;
            uint32_t dst = temp_smem_addr + n * BYTES_PER_ROW;
            asm volatile(
                "cp.async.bulk.shared::cta.global.mbarrier::complete_tx::bytes [%0], [%1], %2, [%3];"
                :: "r"(dst), "l"(src), "r"(BYTES_PER_ROW), "r"(mbar_smem) : "memory"
            );
    ...
    uint32_t& tmem_d, uint32_t& tmem_sfa, uint32_t& tmem_sfb)
{
    ...
    // Initialize MMA mbarrier
    if (threadIdx.x == 0) {
        asm volatile("mbarrier.init.shared::cta.b64 [%0], %1;"
            :: "r"(mbar_mma), "r"(1) : "memory");
        // Initialize TMA mbarrier (arrival count = 1)
        asm volatile("mbarrier.init.shared::cta.b64 [%0], %1;"
...
gemm_kernel_tcgen05(const __grid_constant__ Gemm_params params)
{
    ...
    for (int k_tile = 0; k_tile < num_k_tiles; k_tile++) {
        ...
        // TMA load A (strided global -> contiguous temp)
        tma_load_a_strided(params, m_block, k_offset, a_temp_addr, mbar_tma);
        wait_tma(mbar_tma, tma_phase);

        // TMA load B (strided global -> contiguous temp)
        tma_load_b_strided(params, n_block, k_offset, b_temp_addr, mbar_tma);
        wait_tma(mbar_tma, tma_phase);
...
nvfp4_tcgen05_module = load_inline(
    name="nvfp4_tcgen05_gemm_v3_tma_strided",
    cpp_sources=[cpp_src],
