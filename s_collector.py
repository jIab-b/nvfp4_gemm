...
// ============================================================================
// Constants - MMA_N = 128 version with COLLECTOR BUFFER for A
// ============================================================================
...
// ============================================================================
// MMA with collector buffer support
// collector_mode: 0=fill, 1=use, 2=lastuse
// ============================================================================
__device__ __forceinline__ void issue_mma_collector(
    ...
    int collector_mode)
{
    ...
    if (threadIdx.x == 0) {
        if (collector_mode == 0) {
        ...
        } else if (collector_mode == 1) {
            // Use collector (reuse cached A)
            asm volatile(
                "{"
                ".reg .pred p;"
                "setp.ne.u32 p, %6, 0;"
            ...
            // Last use - read from collector and discard
            asm volatile(
                "{"
                ".reg .pred p;"
                "setp.ne.u32 p, %6, 0;"
...
// ============================================================================
// MAIN KERNEL - with collector buffer for A reuse
// ============================================================================
...
gemm_kernel_tcgen05(const __grid_constant__ Gemm_params params)
{
    ...
    for (int k_tile = 0; k_tile < num_k_tiles; k_tile++) {
        ...
        // Inner loop: 4 MMAs per TMA load, using collector buffer for A reuse
        #pragma unroll
        for (int k_block = 0; k_block < K_BLOCKS; k_block++) {
            ...
            // Collector mode: 0=fill (first), 1=use (middle), 2=lastuse (last)
            // Note: Each k_block has different A data, so we fill each time
            // Collector helps if we were reusing same A across multiple N-tiles
            // For current loop structure, just use fill for first, lastuse for last
            int collector_mode;
            if (k_block == 0) {
                collector_mode = 0;  // fill
            } else if (k_block == K_BLOCKS - 1) {
                collector_mode = 2;  // lastuse
            } else {
                collector_mode = 1;  // use
            }

            issue_mma_collector(tmem_d, a_desc, b_desc, idesc, tmem_sfa, tmem_sfb,
                               mbar_mma, accumulate, collector_mode);
...
nvfp4_tcgen05_module = load_inline(
    name="nvfp4_tcgen05_gemm_collector",
    cpp_sources=[cpp_src],
