...
static constexpr int TMEM_COLS = 144;  // D(128) + SFA(4) + SFB(4) + padding

// Shared Memory Layout
#define SMEM_A_TILE      (MMA_M * MMA_K / 2)  // 4096 bytes
#define SMEM_B_TILE      (MMA_N * MMA_K / 2)  // 4096 bytes
#define SMEM_OFF_MBAR    8
#define SMEM_OFF_TILES   16
...
struct Gemm_params {
    ...
    const void* __restrict__ sfa_perm_ptr;
    const void* __restrict__ sfb_perm_ptr;
...
// ----------------------------------------------------------------------------
// TMEM LOAD SCALE FACTORS (from permuted layout)
// Permuted: [32 lanes, 4 groups, tiles, 4 k_scales, k_tiles, 1], 512 bytes/tile
// ----------------------------------------------------------------------------
__device__ __forceinline__ void load_scales_to_tmem(
    const Gemm_params& params, int k_tile, uint32_t tmem_sfa, uint32_t tmem_sfb)
{
    const int lane_id = threadIdx.x % 32;
    const int warp_id = threadIdx.x / 32;
    const int lane_base = warp_id * 32;

    // SFA: tile offset = (k_tile * num_m_tiles + m_tile) * 512
    const int sfa_offset = (k_tile * gridDim.y + blockIdx.y) * 512 + lane_id + warp_id * 32;
    const uint8_t* sfa = reinterpret_cast<const uint8_t*>(params.sfa_perm_ptr) + sfa_offset;
    uint32_t sfa_val = sfa[0] | (sfa[128] << 8) | (sfa[256] << 16) | (sfa[384] << 24);

    asm volatile(
        "tcgen05.st.sync.aligned.32x32b.x1.b32 [%0], {%1};\\n"
        :: "r"((lane_base << 16) | tmem_sfa), "r"(sfa_val) : "memory"
    );

    // SFB: tile offset = (k_tile * num_n_tiles + n_tile) * 512
    const int sfb_offset = (k_tile * gridDim.x + blockIdx.x) * 512 + lane_id + warp_id * 32;
    const uint8_t* sfb = reinterpret_cast<const uint8_t*>(params.sfb_perm_ptr) + sfb_offset;
    uint32_t sfb_val = sfb[0] | (sfb[128] << 8) | (sfb[256] << 16) | (sfb[384] << 24);

    asm volatile(
        "tcgen05.st.sync.aligned.32x32b.x1.b32 [%0], {%1};\\n"
        :: "r"((lane_base << 16) | tmem_sfb), "r"(sfb_val) : "memory"
    );
...
gemm_kernel_tcgen05(const __grid_constant__ Gemm_params params)
{
    ...
    uint8_t* b_smem = a_smem + SMEM_A_TILE;
    int mbar_phase = 0;

    for (int k_tile = 0; k_tile < params.K / MMA_K; k_tile++) {
        load_a_to_smem(params, m_block, k_tile * MMA_K, a_smem);
        load_b_to_smem(params, n_block, k_tile * MMA_K, b_smem);
        __syncthreads();

        uint64_t a_desc = make_smem_desc(a_smem, 128, 256, 0);
        uint64_t b_desc = make_smem_desc(b_smem, 128, 256, 0);
        load_scales_to_tmem(params, k_tile, tmem_sfa, tmem_sfb);
    ...
    torch::Tensor C)
{
    ...
    Gemm_params params = {
        ...
        SFA_perm.data_ptr(), SFB_perm.data_ptr(),
        reinterpret_cast<__half*>(C.data_ptr()),
        A.stride(0), B.stride(0)
    };

    dim3 grid((N + MMA_N - 1) / MMA_N, (M + MMA_M - 1) / MMA_M);
    gemm_kernel_tcgen05<<<grid, WARPS_PER_CTA * THREADS_PER_WARP>>>(params);
    return C;
}
"""
