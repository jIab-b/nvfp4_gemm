...
// ============================================================================
// TEST: tcgen05.cp variants (for SASS dump inspection)
// ============================================================================
__device__ __forceinline__ void test_tcgen05_cp_variants(
    uint32_t tmem_addr, uint64_t smem_desc)
{
    // All cta_group::1 variants for single-CTA mode
    // .shape = { .128x256b, .4x256b, .128x128b, .64x128b, .32x128b }

    if (threadIdx.x == 0) {
        // Variant 1: .128x256b - largest shape
        asm volatile(
        ...
        asm volatile(
            ...
            :: "r"(tmem_addr + 32), "l"(smem_desc) : "memory"
        );

        // Variant 3: .128x128b
        asm volatile(
            "tcgen05.cp.cta_group::1.128x128b [%0], %1;"
            :: "r"(tmem_addr + 64), "l"(smem_desc) : "memory"
        );

        // Variant 4: .64x128b with multicast (warpx2)
        asm volatile(
            "tcgen05.cp.cta_group::1.64x128b.warpx2::02_13 [%0], %1;"
            :: "r"(tmem_addr + 96), "l"(smem_desc) : "memory"
        );

        // Variant 5: .32x128b with multicast (warpx4)
        asm volatile(
            "tcgen05.cp.cta_group::1.32x128b.warpx4 [%0], %1;"
            :: "r"(tmem_addr + 112), "l"(smem_desc) : "memory"
        );
    }
...
gemm_kernel_tcgen05(const __grid_constant__ Gemm_params params)
{
    ...
    // TEST: Call tcgen05.cp variants once to see SASS output
    // Uses dummy smem descriptor - just for compilation/SASS inspection
    if (blockIdx.x == 0 && blockIdx.y == 0) {
        uint64_t dummy_desc = make_smem_desc(smem + SMEM_OFF_TILES, 16, 256, 0);
        test_tcgen05_cp_variants(tmem_d, dummy_desc);
    }
